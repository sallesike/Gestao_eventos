...
class CertificateController extends Controller
{
    ...
    public function __construct(Certificate $certificate, Subscription $subscription, User $user)
    {
  ...
  {    
    ...
    public function certificateByCpf(UserRequest $request)
    {
        ...
        $user = $this->user;
        $certificate = $this->certificate;
        $user_id = $user->where('cpf', $cpf)->pluck('id')->first();
        if($user_id == null)
        {
    ...
    {        
        ...
        return \PDF::loadView('certificate', compact('certificate'))->setPaper('a4', 'landscape')->stream(kebab_case($certificate->user->name ."-". $certificate->event->name.".pdf"));
    }
